...
LOCATION_GROUPS = {
    "Badges": {loc.label for loc in data.locations.values() if "Badges" in loc.tags},
    "Dexsanity": DEXSANITY_LOCATIONS,
    "Dexcountsanity": DEXCOUNTSANITY_LOCATIONS,
    "Dex": DEXSANITY_LOCATIONS | DEXCOUNTSANITY_LOCATIONS,
    "Hidden Items": {loc.label for loc in data.locations.values() if "Hidden Items" in loc.tags},
    "Item Balls": {loc.label for loc in data.locations.values() if "Item Balls" in loc.tags},
    "Trainersanity": {loc.label for loc in data.locations.values() if "Trainersanity" in loc.tags},
    "Berry Trees": {loc.label for loc in data.locations.values() if "Berry Trees" in loc.tags},
    "Key Items": {loc.label for loc in data.locations.values() if "Key Items" in loc.tags},
    "Ruins of Alph": {loc.label for loc in data.locations.values() if "Ruins of Alph" in loc.tags},
    ...
    "NPC Gifts": {loc.label for loc in data.locations.values() if "NPC Gifts" in loc.tags},
    "Azalea Town": {loc.label for loc in data.locations.values() if "Azalea Town" in loc.tags},
    "Burned Tower": {loc.label for loc in data.locations.values() if "Burned Tower" in loc.tags},
    "Celadon City": {loc.label for loc in data.locations.values() if "Celadon City" in loc.tags},
    "Cerulean City": {loc.label for loc in data.locations.values() if "Cerulean City" in loc.tags},
    "Cianwood City": {loc.label for loc in data.locations.values() if "Cianwood City" in loc.tags},
    "Dark Cave": {loc.label for loc in data.locations.values() if "Dark Cave" in loc.tags},
    "Dragon's Den": {loc.label for loc in data.locations.values() if "Dragon's Den" in loc.tags},
    "Ecruteak City": {loc.label for loc in data.locations.values() if "Ecruteak City" in loc.tags},
    "Saffron City": {loc.label for loc in data.locations.values() if "Saffron City" in loc.tags},
    "Goldenrod City": {loc.label for loc in data.locations.values() if "Goldenrod City" in loc.tags},
    "Ice Path": {loc.label for loc in data.locations.values() if "Ice Path" in loc.tags},
    "Ilex Forest": {loc.label for loc in data.locations.values() if "Ilex Forest" in loc.tags},
    "Lake of Rage": {loc.label for loc in data.locations.values() if "Lake of Rage" in loc.tags},
    "Mount Mortar": {loc.label for loc in data.locations.values() if "Mount Mortar" in loc.tags},
    "National Park": {loc.label for loc in data.locations.values() if "National Park" in loc.tags},
    "Olivine Lighthouse": {loc.label for loc in data.locations.values() if "Olivine Lighthouse" in loc.tags},
    "Olivine City": {loc.label for loc in data.locations.values() if "Olivine City" in loc.tags},
    "Radio Tower": {loc.label for loc in data.locations.values() if "Radio Tower" in loc.tags},
    "Rock Tunnel": {loc.label for loc in data.locations.values() if "Rock Tunnel" in loc.tags},
    "Silver Cave": {loc.label for loc in data.locations.values() if "Silver Cave" in loc.tags},
    "Slowpoke Well": {loc.label for loc in data.locations.values() if "Slowpoke Well" in loc.tags},
    "Sprout Tower": {loc.label for loc in data.locations.values() if "Sprout Tower" in loc.tags},
    "Team Rocket HQ": {loc.label for loc in data.locations.values() if "Team Rocket HQ" in loc.tags},
    "Tin Tower": {loc.label for loc in data.locations.values() if "Tin Tower" in loc.tags},
    "Union Cave": {loc.label for loc in data.locations.values() if "Union Cave" in loc.tags},
    "Vermilion City": {loc.label for loc in data.locations.values() if "Vermilion City" in loc.tags},
    "Victory Road": {loc.label for loc in data.locations.values() if "Victory Road" in loc.tags},
    "Violet City": {loc.label for loc in data.locations.values() if "Violet City" in loc.tags},
    "Whirl Islands": {loc.label for loc in data.locations.values() if "Whirl Islands" in loc.tags},
    "Cherrygrove City": {loc.label for loc in data.locations.values() if "Cherrygrove City" in loc.tags},
    "One offs": {loc.label for loc in data.locations.values() if "One offs" in loc.tags}, #locations with only one check 
    "New Bark Town": {loc.label for loc in data.locations.values() if "New Bark Town" in loc.tags},
    "Blackthorn City": {loc.label for loc in data.locations.values() if "Blackthorn City" in loc.tags},
    "S.S. Aqua": {loc.label for loc in data.locations.values() if "S.S. Aqua" in loc.tags},
    "Fuchsia City": {loc.label for loc in data.locations.values() if "Fuchsia City" in loc.tags},
    "Mahogany Town": {loc.label for loc in data.locations.values() if "Mahogany Town" in loc.tags},
