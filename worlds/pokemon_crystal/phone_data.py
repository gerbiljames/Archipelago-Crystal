...
nidoran_call = PhoneScript(caller_out_of_area, [
    ScriptLine([text_cmd, "PALEX00: Hi " play_g_cmd "!"]),
    ScriptLine([para_cmd, "I wanted to let"]),
    ScriptLine([line_cmd, "you know that I'm"]),
    ScriptLine([para_cmd, "hosting my"]),
    ...
    ScriptLine([para_cmd, "produced the egg,"]),
    ScriptLine([line_cmd, "it has a 50-50"]),
    ScriptLine([para_cmd, "chance to produce"]),
    ScriptLine([line_cmd, "either a female"]),
    ScriptLine([cont_cmd, "or a male NIDORAN!"]),
    ScriptLine([para_cmd, "And that's not"]),
    ScriptLine([line_cmd, "all!"]),
    ScriptLine([para_cmd, "Nidorina and"]),
    ScriptLine([line_cmd, "Nidoqueen are"]),
    ScriptLine([cont_cmd, "fully sterile!"]),
    ScriptLine([para_cmd, "This means"]),
    ScriptLine([line_cmd, "without the"]),
    ScriptLine([para_cmd, "philosophical"]),
    ScriptLine([line_cmd, "nightmare that is"]),
    ScriptLine([para_cmd, "DITTO, the only"]),
    ScriptLine([line_cmd, "canonical way to"]),
    ScriptLine([para_cmd, "breed a NIDORAN"]),
    ScriptLine([line_cmd, "is for a male"]),
    ScriptLine([para_cmd, "Nido like NIDOKING"]),
    ScriptLine([line_cmd, "to bump uglies"]),
    ScriptLine([para_cmd, "with a female"]),
    ScriptLine([line_cmd, "NIDORAN!"]),
    ScriptLine([para_cmd, "Anyways,"]),
    ScriptLine([line_cmd, "tooodeloo!"]),
    ScriptLine([para_cmd, "Click!"]),
    ScriptLine([para_cmd, "…"]),
    ScriptLine([para_cmd, "……"]),
    ScriptLine([para_cmd, "…"]),
    ScriptLine([para_cmd, "……"]),
    ScriptLine([para_cmd, "…"]),
    ScriptLine([para_cmd, "-Ring ring!-"]),
    ScriptLine([para_cmd, "GERBILJAMES: Hi,"]),
    ScriptLine([line_cmd, "James here."]),
    ScriptLine([para_cmd, "Just wanted to"]),
    ScriptLine([line_cmd, "chime in to say"]),
    ScriptLine([para_cmd, "what palex00 said"]),
    ScriptLine([line_cmd, "is correct."]),
    ScriptLine([para_cmd, "I did not believe"]),
    ScriptLine([line_cmd, "it, but I checked"]),
    ScriptLine([cont_cmd, "the code."]),
    ScriptLine([para_cmd, "Does GAMEFREAK"]),
    ScriptLine([line_cmd, "live in fear of"]),
    ScriptLine([cont_cmd, "what they created?"]),
    ScriptLine([done_cmd])
])
